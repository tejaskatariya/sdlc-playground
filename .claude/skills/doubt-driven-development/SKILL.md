...
description: >
  ...
  Use when correctness matters more than speed, when working in unfamiliar code, when stakes
  are high (production, security-sensitive logic, irreversible operations), or any time a
  confident output would be cheaper to verify now than to debug later.
  Consumed by: devils-advocate agent (Mode A — explicitly operationalises this technique at
  team level). Also invokable standalone via /challenge.
---
