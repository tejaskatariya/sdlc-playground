...
Verify conformance to the project constitution (.sdlc-factory/constitution.md,
loaded via CLAUDE.md) — constitution violations are blocking defects, reported
like any other.
...
Artifact-backed issues (an `artifacts: <dir>` body line): the folder's
spec.md and tech-spec.md are the source of truth — verify conformance, and
that every task in tasks.md maps to a commit and is checked off in the
issue's progress comment (the one starting with `<!-- sdlc-factory:progress -->`).
The artifacts are read-only for sessions: ANY change under the artifacts
folder in the PR IS a defect.
